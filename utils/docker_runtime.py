...
class Runtime:
    ...
    def apply_patch(self, patch: str, working_dir: Optional[str] = None, verbose: bool = False) -> bool:
        """Apply ``patch`` via git, falling back through strict, already-applied,
        and 3-way merge before giving up.

        Mirrors OpenHands' ``apply_ground_truth_patch`` in
        ``evaluation/OpenHands/utils.py``: a plain ``git apply -v`` is tried
        first, then ``git apply --reverse --check`` to detect a patch that's
        already present, then ``git apply --3way -v`` for context-drifted but
        index-reachable patches. If all three fail, the worktree is reset to
        HEAD and untracked files are cleaned so the caller sees a known state.

        ``working_dir`` lets the caller pin the patch to a specific repo;
        without it, the shell's current ``pwd`` is used (legacy behavior).
        """
        if not patch:
            return False

        os.makedirs(self.mnt_host, exist_ok=True)
        filename = f"{uuid.uuid4().hex}.diff"
        ...
        apply_cmd = (
            f"{cd_prefix}"
            f"if git apply -v {containerpath}; then echo __SWE_PATCH_APPLIED__; "
            f"elif git apply --reverse --check {containerpath}; then echo __SWE_PATCH_ALREADY_APPLIED__; "
            f"elif git apply --3way -v {containerpath}; then echo __SWE_PATCH_APPLIED_3WAY__; "
            f"else git reset --hard HEAD && git clean -fd && echo __SWE_PATCH_FAILED__; "
            "fi"
        )
        res = self.send_command(apply_cmd)
        try:
            os.remove(hostpath)
        except OSError:
            pass
        ...
        output = res.output
        if "__SWE_PATCH_APPLIED__" in output:
            if verbose:
                print(f"Patch applied cleanly ({filename}).", flush=True)
            return True
        if "__SWE_PATCH_ALREADY_APPLIED__" in output:
            if verbose:
                print(f"Patch was already applied ({filename}).", flush=True)
            return True
        if "__SWE_PATCH_APPLIED_3WAY__" in output:
            if verbose:
                print(f"Patch applied via 3-way merge ({filename}).", flush=True)
            return True
    ...
    def start_session(
        ...
        shell_command = "/bin/bash" if "jefzda" not in image_name else None
        run_kwargs = {
